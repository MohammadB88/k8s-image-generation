...
export default async (fastify: FastifyInstance): Promise<void> => {
  ...
  const jobTracker: Record<string, Payload> = {};
  ...
  fastify.post('/', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      jobTracker[job_id] = data;
      reply.send({ job_id });
    } catch (error) {
      console.error('Error sending request to SDXL endpoint:', error);
      reply.code(500).send({ message: 'Error communicating with the generation endpoint.' });
    }
  });
  ...
  fastify.get('/progress/:job_id', { websocket: true }, (connection, req) => {
    ...
    apiWs.on('message', async (data: WebSocket.Data) => {
      try {
        ...
        // Get the current job details from the jobTracker
        const currentJob = jobTracker[job_id];

        // Check to see if we've passed the threshold for image checking
        if (!currentJob.past_threshold && currentJob.num_inference_steps / 2 < msg.step) {
          currentJob.past_threshold = true;
        }
